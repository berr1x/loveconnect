...
const PartnerMusicPage = () => {
  ...
  const handleFileInput = (e, isFilesInput = false) => {
    if (e.target.files && e.target.files.length > 0) {
      if (
        ...
        alert('Если вы хотите загрузить несколько фото, выберите их через "Файлы" на вашем устройстве, а не через "Фото".');
      }
    ...
    >
      <div className="partner-music-content">
        ...
        >
          ...
          <label htmlFor="partner-files-input" className="upload-button">
            📁 Выбрать из Файлов
          </label>
        </motion.div>
        
        <p className="upload-hint">
          от 3 до 10 скриншотов<br />
          <span style={{ color: '#9D6BFF', fontSize: 13 }}>
            Чтобы выбрать несколько фото, используйте <b>Файлы</b> вместо <b>Фото</b> в галерее вашего телефона.
          </span>
        </p>
